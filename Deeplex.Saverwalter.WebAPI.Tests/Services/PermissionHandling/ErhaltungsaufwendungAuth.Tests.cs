...
{
    public class ErhaltungsaufwendungAuthTests
    {
        [Theory]
        ...
        public async Task HandleAsync(string requirementName, VerwalterRolle? rolle, bool success)
        {
            ...
            var entity = new Erhaltungsaufwendung(100, "test", new DateOnly())
            {
                Wohnung = vertrag.Wohnung,
                Aussteller = vertrag.Ansprechpartner!
            };
            ...
            var permissionHandler = new ErhaltungsaufwendungPermissionHandler();
