    ...
    <StructuredList style="margin: 2em" condensed>
        ...
        <StructuredListBody>
            {#each personenZeitanteile as intervall, index}
                <StructuredListRow>
                    <StructuredListCell>{!index ? einheit.gesamtNutzeinheit : ''}</StructuredListCell>
                    <StructuredListCell>{!index ? convertM2(einheit.gesamtWohnflaeche) : ''}</StructuredListCell>
                    <StructuredListCell>{!index ? convertM2(einheit.gesamtNutzflaeche) : ''}</StructuredListCell>
                    <StructuredListCell>{!index ? einheit.gesamtMiteigentumsanteile : ''}</StructuredListCell>
                    <StructuredListCell>{intervall.gesamtPersonenzahl}</StructuredListCell>
                    <StructuredListCell>
                        {formatDate(intervall.beginn)} – {formatDate(intervall.ende)}
                    </StructuredListCell>
                    <StructuredListCell>{intervall.tage} / {nutzungstage}</StructuredListCell>
                </StructuredListRow>
            {/each}
        </StructuredListBody>
    </StructuredList>
    ...
    <Row>
        <StructuredList condensed style="margin: 2em">
            ...
            <StructuredListBody>
                {#if hasWF}
                    <StructuredListRow>
                        <StructuredListCell head>Wohnfläche (n. WF)</StructuredListCell>
                    </StructuredListRow>
                    <StructuredListRow>
                        <StructuredListCell>
                            {convertM2(zeitanteil > 0 ? (einheit.gesamtWohnflaeche * wfZeitanteil) / zeitanteil : 0)}
                            / {convertM2(einheit.gesamtWohnflaeche)}
                        </StructuredListCell>
                        <StructuredListCell>{formatDate(vonInJahr)} – {formatDate(bisInJahr)}</StructuredListCell>
                        <StructuredListCell>{nutzungstage}</StructuredListCell>
                        <StructuredListCell>{convertPercent(wfZeitanteil)}</StructuredListCell>
                    </StructuredListRow>
                {/if}
                {#if hasNF}
                    <StructuredListRow>
                    ...
                    <StructuredListRow>
                        <StructuredListCell>
                            {convertM2(zeitanteil > 0 ? (einheit.gesamtNutzflaeche * nfZeitanteil) / zeitanteil : 0)}
                            / {convertM2(einheit.gesamtNutzflaeche)}
                        </StructuredListCell>
                        <StructuredListCell>{formatDate(vonInJahr)} – {formatDate(bisInJahr)}</StructuredListCell>
                        <StructuredListCell>{nutzungstage}</StructuredListCell>
                        <StructuredListCell>{convertPercent(nfZeitanteil)}</StructuredListCell>
                    </StructuredListRow>
                {/if}
                {#if hasMEA}
                    <StructuredListRow>
                        <StructuredListCell head>Miteigentumsanteile (n. MEA)</StructuredListCell>
                    </StructuredListRow>
                    <StructuredListRow>
                        <StructuredListCell>
                            {zeitanteil > 0
                                ? ((einheit.gesamtMiteigentumsanteile * meaZeitanteil) / zeitanteil).toFixed(2)
                                : 0}
                            / {einheit.gesamtMiteigentumsanteile}
                        </StructuredListCell>
                        <StructuredListCell>{formatDate(vonInJahr)} – {formatDate(bisInJahr)}</StructuredListCell>
                        <StructuredListCell>{nutzungstage}</StructuredListCell>
                        <StructuredListCell>{convertPercent(meaZeitanteil)}</StructuredListCell>
                    </StructuredListRow>
                {/if}
                {#if hasNE}
                    <StructuredListRow>
                        <StructuredListCell head>Nutzeinheiten (n. NE)</StructuredListCell>
                    </StructuredListRow>
                    <StructuredListRow>
                        <StructuredListCell>
                            {zeitanteil > 0
                                ? ((einheit.gesamtNutzeinheit * neZeitanteil) / zeitanteil).toFixed(2)
                                : 0}
                            / {einheit.gesamtNutzeinheit}
                        </StructuredListCell>
                        <StructuredListCell>{formatDate(vonInJahr)} – {formatDate(bisInJahr)}</StructuredListCell>
                        <StructuredListCell>{nutzungstage}</StructuredListCell>
                        <StructuredListCell>{convertPercent(neZeitanteil)}</StructuredListCell>
                    </StructuredListRow>
                {/if}
                {#if hasPs}
                    <StructuredListRow>
                        <StructuredListCell head>Personenzahl (n. Pers.)</StructuredListCell>
                    </StructuredListRow>
                    {#each personenZeitanteile as p}
                        <StructuredListRow>
                            <StructuredListCell>{p.personenzahl} / {p.gesamtPersonenzahl}</StructuredListCell>
                            <StructuredListCell>{formatDate(p.beginn)} – {formatDate(p.ende)}</StructuredListCell>
                            <StructuredListCell>{p.tage}</StructuredListCell>
                            <StructuredListCell>{convertPercent(p.anteil)}</StructuredListCell>
                        </StructuredListRow>
                    {/each}
                {/if}
            </StructuredListBody>
