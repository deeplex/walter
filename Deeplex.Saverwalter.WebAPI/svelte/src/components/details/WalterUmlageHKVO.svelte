<script lang="ts">
    ...
    import { Checkbox, Link, Row, Slider, Tile } from "carbon-components-svelte";
    import WalterComboBox from "../elements/WalterComboBox.svelte";
    import { WalterUmlageEntry, type WalterHKVOEntry, type WalterSelectionEntry, type WalterWohnungEntry } from "$walter/lib";
    ...
    const hkvo_p9a2 = walter_selection.hkvo_p9a2(fetchImpl).then(
        res => {
    ...
    let selectedWohnungenSet = new Set<number>(selectedWohnungen?.map(e => +e.id));
    function selectedWohnungenChanged()
    {
        const newSet = new Set<number>(selectedWohnungen?.map(e => +e.id));
        if (selectedWohnungenSet.size === newSet.size &&
    ...
    {
        selectableUmlagen = WalterUmlageEntry.GetAll<WalterUmlageEntry>(fetchImpl)
            .then(async (umlagenReponse) => {
                const selectedWohnungenSet = new Set<number>(selectedWohnungen?.map(e => +e.id));
                ...
                const filteredUmlagen = umlagenReponse.filter(umlage =>
                {
                    ...
                    const wohnungenSet = new Set<number>(umlage.selectedWohnungen.map(e => +e.id));
                    // check that only umlagen with the same wohnungen are selectable
                    return selectedWohnungenSet.size === wohnungenSet.size &&
                        [...selectedWohnungenSet].every(id => wohnungenSet.has(id));
                });
...
<Row>
    ...
    <div>
        <Checkbox checked={visible} on:change={(e) => change(e, p9a2)}/>
    </div>
    <p style="margin-top: 0.25em; margin-left: -0.5em">Ist diese Umlage nach der
    <Link href="https://www.gesetze-im-internet.de/heizkostenv/">Heizkostenverordnung</Link>
     umzulegen?</p>
    {/await}
</Row>
{#if entry.hKVO}
