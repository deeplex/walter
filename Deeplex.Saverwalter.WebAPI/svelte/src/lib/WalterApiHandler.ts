...
export abstract class WalterApiHandler {
    ...
    public static async GetAll<T extends WalterApiHandler>(fetch: any): Promise<T[]> {
        const response = await walter_get(this.ApiURL, fetch);

        return response.map(this.fromJson);
    }

    public static async GetOne<T extends WalterApiHandler>(id: string, fetch: any): Promise<T> {
