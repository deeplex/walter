import { WalterAdresseEntry, WalterPersonEntry, WalterSelectionEntry, WalterVertragEntry, WalterWohnungEntry } from "$WalterLib";
import { WalterApiHandler } from "./WalterApiHandler";

export class WalterJuristischePersonEntry extends WalterApiHandler implements WalterPersonEntry {
    ...
    static fromJson(json: any) {
        const selectedMitglieder = json.selectedMitglieder
            ?.map(WalterSelectionEntry.fromJson);
        const mitglieder = json.mitglieder
            ?.map(WalterSelectionEntry.fromJson);
