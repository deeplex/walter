import { WalterMieteEntry, WalterMietminderungEntry, WalterPersonEntry, WalterSelectionEntry, WalterVertragVersionEntry } from "$WalterLib";
...
export class WalterVertragEntry extends WalterApiHandler {
    ...
    static fromJson(json: any) {
        ...
        const ansprechpartner = json.ansprechpartner
            && WalterSelectionEntry.fromJson(json.ansprechpartner);
        const selectedMieter = json.selectedMieter
            ?.map(WalterSelectionEntry.fromJson);
        const versionen = json.versionen
            ?.map(WalterVertragVersionEntry.fromJson);
        const mieter = json.mieter
            ?.map(WalterPersonEntry.fromJson);
        const mieten = json.mieten
            ?.map(WalterMieteEntry.fromJson);
        const mietminderungen = json.mietminderungen
            ?.map(WalterMietminderungEntry.fromJson);
