import { WalterAdresseEntry, WalterBetriebskostenrechnungEntry, WalterErhaltungsaufwendungEntry, WalterSelectionEntry, WalterUmlageEntry, WalterVertragEntry, WalterZaehlerEntry } from "$WalterLib";
...
export class WalterWohnungEntry extends WalterApiHandler {
    ...
    static fromJson(json: any) {
        ...
        const besitzer = json.besitzer
            && WalterSelectionEntry.fromJson(json.besitzer);
        const haus = json.haus
            ?.map(WalterWohnungEntry.fromJson);
        const zaehler = json.zaehler
            ?.map(WalterZaehlerEntry.fromJson);
        ...
        const erhaltungsaufwendungen = json.erhaltungsaufwendungen
            ?.map(WalterErhaltungsaufwendungEntry.fromJson);
        const umlagen = json.umlagen
            ?.map(WalterUmlageEntry.fromJson);
