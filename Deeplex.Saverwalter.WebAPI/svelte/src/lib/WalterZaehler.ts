import { WalterAdresseEntry, WalterSelectionEntry, WalterZaehlerstandEntry } from "$WalterLib";
...
export class WalterZaehlerEntry extends WalterApiHandler {
    ...
    static fromJson(json: any) {
        ...
        const allgemeinZaehler = json.allgemeinZaehler
            && WalterSelectionEntry.fromJson(json.allgemeinZaehler);
        const wohnung = json.wohnung
            && WalterSelectionEntry.fromJson(json.wohnung);
        const staende = json.staende
            ?.map(WalterZaehlerstandEntry.fromJson);
        const einzelzaehler = json.einzelzaehler
