...
function getZeitanteil(
    rechnungsgruppe: WalterBetriebskostenabrechnungsRechnungsgruppe,
    ...
    switch (umlage.schluessel.id) {
        ...
        case '2':
            return rechnungsgruppe.personenZeitanteil.reduce(
                (pre, cur) => pre + cur.anteil,
                0
            );
        case '3':
            console.warn(
                `Anzeige nach Verbrauch ist noch nicht implementiert. Dies ist nur vielleicht korrekt`
            );
        ...
        default:
            console.error(
                `Umlageschlüssel ${umlage.schluessel.text} kann nicht zugeordnet werden.`
            );
            return 0;
    }
}
...
export function getKostengruppen(
    ...
    return abrechnung.abrechnungseinheiten.map((abrechnungseinheit) => {
        const kostenpunkte = abrechnungseinheit.umlagen.map(
            (umlageEntry, index) =>
                getKostenpunkt(
                    index,
                    umlageEntry,
                    new Date(abrechnung.nutzungsbeginn).toLocaleDateString(
                        'de-De'
                    ),
                    new Date(abrechnung.nutzungsende).toLocaleDateString(
                        'de-De'
                    ),
                    abrechnung.jahr,
